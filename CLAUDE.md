...
| `view/menuDom.js` + `view/menu.css` | **Nové menu v HTML nad plátnem** (převádí se po obrazovkách, plán a stav fází: [docs/menu-ui-plan.md](docs/menu-ui-plan.md)). Převedené obrazovky drží `MENU_DOM_SCREENS` (menu mimo místnost) a `MENU_DOM_ROOM_PHASES` (lobby S8/S9 podle `roomPhase`), k tomu konec hry (`winner` = S12, `next_game` = S13 účast v další hře, `stats` = S14, když stav má `winner`); kterou z nich vrstva právě kreslí, říká `menuDomScreen()`; ostatní kreslí dál Phaser (`view/menu.js`). Text od hráče vždy přes `esc()` (core/menuModel.js). Vzhled se dá ověřit snímkem: `node tools/menushot.js`. |
