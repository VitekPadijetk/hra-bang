...
Hotové jsou **fáze 0–9**: balíček je ve hře a **osm z deseti karet událostí** už něco
dělá (včetně té vespod), z osmi postav chybí jen **Greygory Deck**. Zbývají fáze 10–12:
`hasAbility` + Greygory Deck, Lady Růže z Texasu a Zuřivá Doroty.
...
- **Postavy 34–41** jsou v `characters.json` i jako `WILD_WEST_CHARACTERS`
  ([logic/entities.js](logic/entities.js)) včetně portrétů a životů. Do OSTRÉ hry jdou
  po jedné, jak přibývají schopnosti: `_characterPool` bere `WILD_WEST_READY` (dnes
  všechny kromě Greygoryho Decka), debug hra dál všech osm. Chybí Greygory Deck (fáze 10).
- **`effectiveCharacter` se rozdělí na `hasAbility`** (Greygory Deck má dvě schopnosti
  naráz). Portrét a zobrazení se ptají dál `effectiveCharacter`, pravidla přes `hasAbility`.
