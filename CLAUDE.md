...
| `view/menuDom.js` + `view/menu.css` | **Nové menu v HTML nad plátnem** (převádí se po obrazovkách, plán a stav fází: [docs/menu-ui-plan.md](docs/menu-ui-plan.md)). Převedené obrazovky drží `MENU_DOM_SCREENS`, ostatní kreslí dál Phaser (`view/menu.js`). Text od hráče vždy přes `esc()` (core/menuModel.js). Vzhled se dá ověřit snímkem: `node tools/menushot.js`. |
...
## Rozšíření Zlatá horečka (Gold Rush) — hotové fáze 0–3 z 8
...
prací (§9) a rozhodnutími R1–R15. **Assety chybí celé** (§2.8), takže se rozšíření dá
odpracovat celé, ale ne zahrát očima; fáze 0–6 jsou napsané tak, aby art nepotřebovaly.
...
**Panák**, **Union Pacific**, **Boty**, **Talisman**, **Opasek**, **Krumpáč**, **Kalumet**,
**Podkova**, **Rýžovací pánev** a **Batoh**; zbylých pět druhů i všech 8 postav přijde
ve fázích 4–6.
...
- **Art karet vybavení chybí celý**, takže se každý druh vysází jako čitelný ŠTÍTEK
...
- **Šest pasivních karet fáze 2 nemá vlastní metodu** — každá jen doplnila podmínku
  do háku, který už existoval: **Boty** a **Talisman** do `_afterLifeLost`, **Opasek**
...
- **Fáze `GEAR_TARGET`** (Panák: „hráč dle tvé volby, i ty") je jediná fáze, kterou obchod
  má — nákup sám fází NENÍ (rozhodnutí R6). Cíl se vybírá klikem na postavu soupeře
  (stejný vzor jako Pokrevní bratři); na SEBE je tlačítko, protože vlastní portrét se
  v mojí zóně neklika.
- **Placené vybavení (fáze 3) se používá KLIKEM NA KARTU** v mém pásu vybavení, stejně
  ...
  - **Rýžovací pánev** (`gearPanUse`) za 1 valoun otevře běžnou fázi lízání mimo začátek
